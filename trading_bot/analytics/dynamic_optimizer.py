...
class DynamicOptimizer:
    ...
    def __init__(self, data_path: str = "data/optimizer_data.json"):
        ...
        self.regime_parameters = {
            ...
            "sideways": OptimalParameters(
                confidence_threshold=0.55,
                rsi_period=18,  # More responsive (was 21)
                ema_fast=5,
                ema_slow=13,
                macd_fast=8,
                macd_slow=21,
                ...
                take_profit_multiplier=1.8  # Better R:R (was 1.5)
            ),
    ...
    def detect_market_regime(self, price_data: np.ndarray, volume_data: np.ndarray, lookback: int = 50) -> MarketRegime:
        ...
        try:
            ...
            # 4. REGIME CLASSIFICATION (Ultra-sensitive thresholds)
            if volatility > 0.08:  # Very low volatility threshold (was 0.15)
                regime_type = "volatile"
                strength = min(volatility / 0.2, 1.0)
            elif trend_strength > 0.001 and ma_strength > 0.0005:  # Ultra-sensitive trending (was 0.005, 0.002)
                if ma_trend > 0:
                    regime_type = "trending_up"
                else:
                    regime_type = "trending_down"
                strength = min((trend_strength + ma_strength) / 0.003, 1.0)
            else:  # Sideways market
                regime_type = "sideways"
                strength = 1.0 - min(trend_strength / 0.001, 1.0)
