...
class TradingPipeline:
    ...
    def __init__(
        ...
        self._positions_cache_path = Path("data/bot_positions.json")  # Persist positions across restarts
        self._last_reconciliation_time = 0  # Throttle reconciliation to every 60 seconds
    ...
    def _load_existing_positions(self) -> None:
        """Load existing positions from exchange on startup - BOTH balance holdings AND open orders."""
        try:
            ...
            logger.info("📊 STEP 1: Loading positions from balance...")
            for asset, amount in balance["free"].items():
                ...
                try:
                    ...
                    # Try to get current price - NO FALLBACK
                    try:
                        ticker = self._okx.fetch_ticker(symbol)
                        current_price = float(ticker["last"])
                    except Exception as price_exc:
                        logger.error("❌ Could not get price for %s: %s - SKIPPING asset (no fallback)", asset, price_exc)
            ...
            # STEP 2: Load open orders from exchange (ONLY limit/market orders, NOT algo orders)
                    ...
                    for order in regular_orders:
                        try:
                            ...
                            if symbol in self._positions:
                                logger.debug("🔍 Symbol %s already loaded from balance, skipping open order", symbol)
                                continue
                            ...
                            if amount <= 0 or price <= 0:
                                logger.debug("🔍 Skipping invalid open order for %s: amount=%.6f, price=%.6f", symbol, amount, price)
                                continue
                            
                            position_value = amount * price
                            
                            if position_value >= min_position_value:
                                logger.info("✅ OPEN ORDER FOUND: %s - %.6f tokens @ $%.6f = $%.2f (Status: %s)", 
                                           symbol, amount, price, position_value, status)
                                
                                # Create position record for open order
                                atr_multiplier = 0.02
                                stop_loss = price * (1 - atr_multiplier)
                                take_profit = price * (1 + atr_multiplier * 2)
                                ...
                                logger.info("📊 OPEN ORDER POSITION LOADED: %s - %.6f tokens (~$%.2f)", 
                                           symbol, amount, position_value)
                            else:
                                logger.info("❌ OPEN ORDER TOO SMALL: %s - $%.2f (below $%.2f threshold)", 
                                           symbol, position_value, min_position_value)
                        except Exception as exc:
                            logger.warning("❌ ERROR LOADING OPEN ORDER: %s", exc)
                            continue
            except Exception as exc:
                logger.warning("⚠️ Could not fetch open orders: %s", exc)
    ...
    def _load_persisted_positions(self) -> None:
        """Load positions directly from exchange TP/SL orders - NO persisted file."""
        try:
            # Get open algo orders (TP/SL protection) directly from OKX API
            algo_symbols = {}  # symbol -> algo_order_data
            try:
                # Fetch OCO orders (TP/SL protection)
                logger.info("🔍 Fetching active TP/SL orders from OKX API...")
                oco_response = self._okx.fetch_algo_orders("oco")
                oco_data = oco_response.get("data", [])
                
                logger.info("🔍 Found %d OCO (TP/SL) orders on exchange", len(oco_data))
                ...
                for order in oco_data:
                    ...
                    if inst_id:
                        ...
                        algo_symbols[symbol] = order
                        logger.debug("🔍 Found TP/SL order for: %s", symbol)
                
                logger.info("🔍 Active TP/SL symbols: %s", list(algo_symbols.keys()))
                
            except Exception as exc:
                logger.warning("⚠️ Could not fetch algo orders from OKX API: %s", exc)
                return
            
            loaded_count = 0
            
            # Load ALL positions from exchange that have TP/SL orders
            for symbol in algo_symbols:
                if symbol in self._positions:
                    logger.debug("🔍 Symbol %s already loaded, skipping", symbol)
                    continue
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning("⚠️ Could not get price for %s: %s - SKIPPING", symbol, price_exc)
                        continue
                    ...
                    if asset_balance <= 0:
                        logger.warning("⚠️ SKIPPING POSITION: %s - NO BALANCE", symbol)
                        continue
                    ...
                    position = Position(
                        ...
                        take_profit=take_profit,
                        order_id=None,
                        protection_algo_id=algo_symbols[symbol].get("algoId"),
                    ...
                    self._positions[symbol] = position
                    loaded_count += 1
                    logger.info("✅ POSITION LOADED: %s - %.6f tokens @ $%.6f (from exchange TP/SL orders)", 
                               symbol, asset_balance, current_price)
                    
                except Exception as exc:
                    logger.warning("❌ ERROR LOADING POSITION: %s - %s", symbol, exc)
                    continue
            
            logger.info("📂 POSITIONS LOADED: %d from exchange TP/SL orders", loaded_count)
        except Exception as exc:
            logger.error("Failed to load positions from exchange: %s", exc)
    ...
    def _reconcile_positions_with_exchange(self, force: bool = False) -> None:
        ...
        IMPORTANT: Only remove positions that are DEFINITELY closed (0 balance + no open order)
        Do NOT remove positions just because balance is slightly different (fees, rounding)
        ...
        logger.debug("🔄 RECONCILING POSITIONS with exchange state...")
        
        try:
            # Get current balance to check for actual holdings
            balance = self._okx.fetch_balance()
            if not balance or "free" not in balance:
                logger.warning("⚠️ Could not fetch balance for reconciliation")
                return
            
            free_assets = balance.get("free", {})
            
            # Get all open orders (regular limit/market) to check for pending orders
            try:
                open_orders = self._okx.fetch_open_orders()
            except Exception as exc:
                logger.warning("⚠️ Could not fetch open orders for reconciliation: %s", exc)
                open_orders = []
            ...
            try:
                ...
                logger.warning("⚠️ Could not fetch algo orders for reconciliation: %s", exc)
            ...
            for symbol, position in list(self._positions.items()):
                try:
                    ...
                    # CRITICAL: Only remove if DEFINITELY closed
                    # Position is closed ONLY if:
                    # 1. Asset balance is ZERO (not just low)
                    # 2. AND no open order exists
                    # This prevents false removals due to fees or rounding
                    if asset_balance <= 0 and not has_open_order:
                        ...
                        logger.debug("✅ POSITION ACTIVE: %s - Balance: %.6f, Amount: %.6f, Open Order: %s", 
                                    symbol, asset_balance, position.amount, has_open_order)
                        
                except Exception as exc:
                    logger.warning("⚠️ Error reconciling position %s: %s", symbol, exc)
                    continue
            ...
            if positions_to_remove:
                ...
                logger.debug("✅ ALL POSITIONS RECONCILED: %d positions active", len(self._positions))
    ...
    def _handle_pending_sell_orders(self) -> None:
        ...
        try:
            ...
            if not regular_sell_orders:
                return
            
            logger.info("🔍 Found %d pending sell orders to check (excluded algo/OCO orders)", len(regular_sell_orders))
    ...
    def run_cycle(self, symbol: str) -> TradeResult:
        ...
        with PerformanceTimer("pipeline", "run_cycle"):
            ...
            # PRE-CHECK: Validate data availability before advanced analytics
            mtf_data = self._market_data.get_multi_timeframe_data(symbol)
            if not mtf_data:
                logger.warning("🚨 SKIPPING %s: No multi-timeframe data available", symbol)
                return TradeResult(symbol, "HOLD", False, None)
            ...
            if not candles_5m or len(candles_5m) < 50:
                ...
                logger.warning("🚨 SKIPPING %s: Insufficient data (%d candles, need 50+)", symbol, candle_count)
                return TradeResult(symbol, "HOLD", False, None)
            
            logger.info("✅ DATA VALIDATED: %s has %d candles - PROCEEDING WITH ADVANCED ANALYTICS", symbol, len(candles_5m))
            ...
            try:
                ...
                # Skip symbol if multi-timeframe analysis failed (no fake data)
                if mtf_signal is None:
                    logger.error("❌ SKIPPING %s - Multi-timeframe analysis failed (no real data available)", symbol)
                    return TradeResult(symbol, "HOLD", False, None)
                ...
                try:
                    ...
                    if mtf_data:
                        ...
                        if candles_5m and len(candles_5m) >= 50:
                            ...
                            logger.info("📊 MARKET REGIME: %s - %s (strength=%.2f, volatility=%.2f) [%d candles]",
                                       symbol, market_regime.regime_type, market_regime.strength, market_regime.volatility, len(candles_5m))
                            ...
                            candle_count = len(candles_5m) if candles_5m else 0
                            logger.warning("🚨 INSUFFICIENT DATA: %s has only %d candles (need 50+) - SKIPPING REGIME DETECTION", 
                                         symbol, candle_count)
                            optimal_params = None
                            market_regime = None
                    else:
                        logger.warning("🚨 NO MTF DATA: %s has no multi-timeframe data - ADVANCED ANALYTICS DISABLED", symbol)
                        optimal_params = None
                        market_regime = None
                except Exception as opt_exc:
                    logger.warning("Dynamic optimization failed for %s: %s", symbol, opt_exc)
                    optimal_params = None
                    market_regime = None
                ...
                try:
                    ...
                    if candles and len(candles) >= 50:
                        ...
                        logger.info("🏗️ MARKET STRUCTURE: %s - trend=%s, smart_money=%s, strength=%.2f [%d candles]",
                                   symbol, market_structure.trend_structure, 
                                   market_structure.smart_money_direction, market_structure.structure_strength, len(candles))
                        ...
                        logger.warning("🚨 INSUFFICIENT DATA: %s has only %d candles (need 50+) - SKIPPING MARKET STRUCTURE", 
                                     symbol, candle_count)
                        market_structure = None
                except Exception as struct_exc:
                    logger.warning("Market structure analysis failed for %s: %s", symbol, struct_exc)
                    market_structure = None
    ...
    def _execute_buy_order(
        ...
        try:
            ...
            if algo_id:
                logger.info("✅ OCO PROTECTION ACTIVE: %s - Exchange managing SL/TP (Algo ID: %s)", symbol, algo_id)
            else:
                logger.warning("⚠️ OCO PROTECTION FAILED: %s - Falling back to manual SL/TP monitoring", symbol)
            ...
            # CRITICAL: Save positions to file for persistence
            self._save_positions()
    ...
    def _gather_market_state(self, symbol: str) -> MarketState:
        ...
        prices: list[float] = []
        try:
            ohlcv = self._okx.fetch_ohlcv(symbol, timeframe="1m", limit=300)
    ...
    def _execute_decision(
        ...
        if decision == "SELL" and existing_position:
            ...
            if order:
                del self._positions[symbol]
                # CRITICAL: Save positions to file after deletion
                self._save_positions()
                return True, None
            return False, None
    ...
    def _size_position(
        ...
        *,
        ...
        try:
            ...
            logger.error("❌ POSITION SIZING FAILED for %s: %s - NO fallback, SKIPPING", symbol, exc)
            return None
    ...
    def _place_protection_orders(
        ...
        *,
        ...
        logger.info("🛡️ STARTING OCO PLACEMENT: %s - Amount: %.6f, SL: %.6f, TP: %.6f, Entry: %.6f", 
                   symbol, amount, stop_loss, take_profit, entry_price or 0)
        ...
        try:
            ...
            logger.warning("⚠️ Could not get amount precision for %s: %s - using raw amount", symbol, exc)
            sz = str(amount)

        # Use entry_price if provided, otherwise use stop_loss/take_profit as-is
        if not entry_price or entry_price <= 0:
            logger.warning("⚠️ No valid entry price for %s OCO; using calculated levels", symbol)
        ...
        if take_profit <= entry:
            ...
            logger.warning("⚠️ TP adjusted: %.6f -> %.6f (was <= entry)", original_tp, take_profit)
        if stop_loss >= entry:
            stop_loss = entry - min_tick
            logger.warning("⚠️ SL adjusted: %.6f -> %.6f (was >= entry)", original_sl, stop_loss)
        if stop_loss <= 0:
            logger.error("❌ STOP-LOSS INVALID: %.6f <= 0 for %s - SKIPPING OCO", stop_loss, symbol)
            return None
        ...
        try:
            ...
            logger.debug("✅ Price precision: TP=%s, SL=%s", tp_trigger, sl_trigger)
        except Exception as exc:
            logger.warning("⚠️ Could not get price precision for %s: %s - using raw prices", symbol, exc)
            tp_trigger = str(take_profit)
            sl_trigger = str(stop_loss)
        ...
        logger.debug("🔍 OCO RESPONSE CODE: %s, Message: %s", sCode, sMsg)
        ...
        if sCode not in {None, "0"}:
            ...
            if sCode == "51155":
                logger.warning("⚠️ Symbol restricted: %s", symbol)
                self._mark_symbol_restricted(symbol)
            return None

        algo_id = entry_data.get("algoId")
        if not algo_id:
            logger.error("❌ MISSING ALGO ID: %s - Response entry: %s", symbol, entry_data)
            return None
    ...
    def _extract_entry_price(self, order: dict[str, Any], default: Optional[float] = None) -> Optional[float]:
        """Extract entry price from order - returns default if no real data."""
        try:
            ...
            return default  # Return default if no price
        except (TypeError, ValueError):
            return default  # Return default on error

    def _get_tick_size(self, symbol: str) -> Optional[float]:
        """Get tick size from market - returns None if no real data."""
        try:
    ...
    def _calculate_trade_levels(
        ...
        try:
            ...
            mtf_data = self._market_data.get_multi_timeframe_data(symbol)
            if not mtf_data:
                logger.error("❌ NO MULTI-TIMEFRAME DATA for %s - NO fallback", symbol)
                return None, None
            ...
            logger.error("❌ TECHNICAL ANALYSIS FAILED for %s: %s - NO fallback, SKIPPING", symbol, exc)
            return None, None
