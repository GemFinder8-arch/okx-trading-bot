    ...
    def _load_persisted_positions(self) -> None:
        ...
        try:
            ...
            if not data or not isinstance(data, dict):
                logger.warning("⚠️ Invalid persisted positions data")
                return
            ...
            for symbol, pos_data in data.items():
                try:
                    ...
                    if symbol in self._positions:
                        logger.debug("🔍 Symbol %s already loaded from exchange, skipping persisted", symbol)
                        continue
                    
                    # Reconstruct position from persisted data
                    position = Position(
                        symbol=symbol,
                    ...
                    self._positions[symbol] = position
                    loaded_count += 1
                    logger.info("✅ PERSISTED POSITION LOADED: %s - %.6f tokens @ $%.6f", 
                               symbol, position.amount, position.entry_price)
                except Exception as exc:
                    logger.warning("❌ ERROR LOADING PERSISTED POSITION: %s - %s", symbol, exc)
                    continue
